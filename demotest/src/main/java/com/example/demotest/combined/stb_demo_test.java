...
public class stb_demo_test {
    ...
    public void Test1() throws InterruptedException {
        ...
        System.out.println("[STB] 實境旅行體驗區測試 開始");
        Reporter.log("[STB] 實境旅行體驗區測試 開始");
        ...
        String elText1 = eldemo.getText();
        System.out.println("欄位名稱:" + elText1);

        if(elText1.equals("體驗區"))
        {
        ...
        Thread.sleep(3000);
        System.out.println("[STB] 確認體驗區可以開始出團");
        Reporter.log("[STB] 確認體驗區可以開始出團");
        MobileElement el_demo_travel = (MobileElement) driver.findElementByXPath("/hierarchy/android.widget.FrameLayout/android.widget.LinearLayout/android.widget.FrameLayout/android.widget.RelativeLayout/android.webkit.WebView/android.webkit.WebView/android.view.View[2]/android.view.View[1]/android.view.View[2]/android.widget.Image[5]");
        wait = new WebDriverWait(driver, 20);
        if (wait.until(ExpectedConditions.visibilityOf(el_demo_travel)) != null) {
            System.out.println("[STB] 體驗區正常");
            Reporter.log("[STB] 體驗區正常");
        } else {
            System.out.println("[STB] 體驗區異常");
            Reporter.log("[STB] 體驗區異常");
            isTestPass = false;
        }
        ...
        MobileElement el_demo_wait = (MobileElement) driver.findElementByXPath("/hierarchy/android.widget.FrameLayout/android.widget.LinearLayout/android.widget.FrameLayout/android.widget.LinearLayout/android.widget.FrameLayout/android.view.ViewGroup/android.widget.FrameLayout/android.view.ViewGroup/android.widget.RelativeLayout[5]/android.widget.RelativeLayout[2]");
        wait = new WebDriverWait(driver, 20);
        if (wait.until(ExpectedConditions.visibilityOf(el_demo_wait)) != null) {
        ...
        Thread.sleep(3000);
        System.out.println("[STB] 檢查導遊影片有沒有出現");
        ...
        if (wait.until(ExpectedConditions.visibilityOf(el_travel)) != null) {
            System.out.println("[STB] 有出現");
            Reporter.log("[STB] 有出現");
        } else {
            System.out.println("[STB] 沒有出現");
            Reporter.log("[STB] 沒有出現");
            isTestPass = false;
        }
        ...
        System.out.println("[STB] 檢查有沒有成功發送預設訊息");
        MobileElement elmessage = (MobileElement) driver.findElementByXPath("/hierarchy/android.widget.FrameLayout/android.widget.LinearLayout/android.widget.FrameLayout/android.widget.LinearLayout/android.widget.FrameLayout/android.view.ViewGroup/android.widget.RelativeLayout/android.widget.RelativeLayout[2]/androidx.recyclerview.widget.RecyclerView/android.view.ViewGroup[2]/android.widget.TextView[2]");
        WebDriverWait wait2 = new WebDriverWait(driver, 300);
        wait2.until(ExpectedConditions.visibilityOf(elmessage));
        System.out.println("[STB] [PASS] 預設訊息發送成功");
        Reporter.log("[STB] [PASS] 預設訊息發送成功");
        ...
        wait3.until(ExpectedConditions.visibilityOf(elvoice));
        ...
        wait4.until(ExpectedConditions.visibilityOf(notificationPage));
        ...
        wait5.until(ExpectedConditions.visibilityOf(travel_end));
        ...
        System.out.println("[STB] 體驗區測試完成 時間總計 : " + diffTime2 + " 秒");
        Reporter.log("[STB] 體驗區測試完成 時間總計 : " + diffTime2 + " 秒");
